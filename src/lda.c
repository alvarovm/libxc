...
xc_lda_initalize(const xc_func_type *func, size_t np, xc_lda_out_params *out)
{
  ...
  /* initialize output */
  if(out->zk != NULL)
    memset(out->zk,     0, np*sizeof(double)*dim->zk);

  if(out->vrho != NULL)
    memset(out->vrho,   0, np*sizeof(double)*dim->vrho);

  if(out->v2rho2 != NULL)
    memset(out->v2rho2, 0, np*sizeof(double)*dim->v2rho2);

  if(out->v3rho3 != NULL)
    memset(out->v3rho3, 0, np*sizeof(double)*dim->v3rho3);

  if(out->v4rho4 != NULL)
    memset(out->v4rho4, 0, np*sizeof(double)*dim->v4rho4);
}
