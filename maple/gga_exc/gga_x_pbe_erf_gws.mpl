...
(* second part of eq. (3)*)
pbe_x_erf_gws_Fx := (rs,z,s) -> 1 + pbe_x_erf_gws_kappa_fx(rs,z)*(1 - pbe_x_erf_gws_kappa_fx(rs,z)/(pbe_x_erf_gws_kappa_fx(rs,z) + pbe_x_erf_gws_b_mod(lda_x_sr_a(rs,z))*s^2)):


(* first part of eq. (3)*)
f := (rs, z, xt, xs0, sx1) -> lda_x_sr_f(rs,z) * pbe_x_erf_gws_Fx(rs,z,xs0*X2S):
